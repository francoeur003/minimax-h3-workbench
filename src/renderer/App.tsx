...
const initialRequest: GenerationRequest = { mode: "text", backend: "seedance", prompt: demoPrompt, duration: 4, ratio: "16:9", resolution: "768P", width: 1280, height: 720, count: 4, baseSeed: Math.floor(Math.random() * 1_000_000) };
...
function ReadyPage({ report, busy, onDetect, onNavigate }: { report?: EnvironmentReport; busy: boolean; onDetect: () => void; onNavigate: (page: Page) => void }) {
  ...
  return <section className="page ready-page"><header className="hero"><span className="eyebrow">SYSTEM READINESS</span><h1>先判断怎么跑，<br/><em>再开始生成。</em></h1><p>一次检查本机硬件、存储、FFmpeg 和 ComfyUI，给出本地、SSH 或云 API 的明确建议。</p><button className="primary large" onClick={onDetect} disabled={busy}>{busy ? "正在检测…" : "开始配置检测"}</button></header>
    ...
    <div className="quick-actions"><button onClick={() => onNavigate("downloads")}><span>01</span><strong>下载模型</strong><small>点击直达官方文件下载</small></button><button onClick={() => onNavigate("connections")}><span>02</span><strong>配置连接</strong><small>本地 / Seedance / H3 / SSH</small></button><button onClick={() => onNavigate("studio")}><span>03</span><strong>开始生成</strong><small>四路结果并行管理</small></button></div>
  </section>;
}
...
function StudioPage({ settings, tasks, onError, onNotice }: { settings: AppSettings; tasks: GenerationTask[]; onError: (e: unknown) => void; onNotice: (s: string) => void }) {
  ...
  return <section className="page studio-page"><div className="page-title"><div><span className="eyebrow">CREATE</span><h1>生成工作台</h1></div><div className="backend-pill"><span />{backendLabel(request.backend)}</div></div>
    <div className="studio-layout"><div className="control-panel"><div className="mode-tabs">{(["text", "image", "video"] as GenerationMode[]).map((mode) => <button key={mode} className={request.mode === mode ? "active" : ""} onClick={() => setRequest({ ...request, mode })}>{mode === "text" ? "文生视频" : mode === "image" ? "图生视频" : "视频生视频"}</button>)}</div>
      ...
      <div className="form-grid"><label>生成后端<select value={request.backend} onChange={(e) => setRequest({ ...request, backend: e.target.value as BackendKind })}><option value="seedance">Seedance 2.0 API</option><option value="local">本机 ComfyUI</option><option value="ssh">SSH 远程显卡</option><option value="minimax">MiniMax H3 云 API</option></select></label><label>时长<select value={request.duration} onChange={(e) => setRequest({ ...request, duration: Number(e.target.value) })}>{[4,6,8,10,12,15].map((x) => <option key={x} value={x}>{x} 秒</option>)}</select></label><label>画幅<select value={request.ratio} onChange={(e) => setRequest({ ...request, ratio: e.target.value as GenerationRequest["ratio"] })}>{["16:9","9:16","1:1","4:3","3:4","21:9","adaptive"].map((x) => <option key={x}>{x}</option>)}</select></label><label>分辨率{request.backend === "seedance" ? <select value="480P" disabled><option>480P</option></select> : <select value={request.resolution} onChange={(e) => setRequest({ ...request, resolution: e.target.value as "768P" | "2K" })}><option>768P</option><option>2K</option></select>}</label></div>
      <label>基础随机种子<input type="number" value={request.baseSeed} onChange={(e) => setRequest({ ...request, baseSeed: Number(e.target.value) })}/></label>
      <div className="submit-area"><div>{request.backend === "minimax" ? <><span>云端估算</span><strong>约 ${estimateCloudCost(request.resolution, request.duration, 4).toFixed(2)} / 4 条</strong></> : request.backend === "seedance" ? <><span>Seedance 2.0</span><strong>按账户点数计费 · 4 路并发</strong></> : <><span>本地生成</span><strong>不产生 API 费用</strong></>}</div><button className="primary" disabled={submitting} onClick={submit}>{submitting ? "提交中…" : "生成 4 个结果"}</button></div>
...
function ConnectionsPage({ settings, setSettings, onError, onNotice }: { settings: AppSettings; setSettings: (s: AppSettings) => void; onError: (e: unknown) => void; onNotice: (s: string) => void }) {
  const [draft, setDraft] = useState(settings); const [apiKey, setApiKey] = useState(""); const [seedanceUsername, setSeedanceUsername] = useState(""); const [seedancePassword, setSeedancePassword] = useState(""); const [sshPassword, setSshPassword] = useState(""); const [testing, setTesting] = useState<BackendKind>(); const [results, setResults] = useState<Partial<Record<BackendKind, BackendTestResult>>>({});
  const setSsh = (patch: Partial<AppSettings["ssh"]>) => setDraft({ ...draft, ssh: { ...draft.ssh, ...patch } });
  async function save() { try { const saved = await must(window.h3.updateSettings(draft)); if (apiKey) await must(window.h3.setSecret("minimaxApiKey", apiKey)); if (seedanceUsername) await must(window.h3.setSecret("seedanceUsername", seedanceUsername)); if (seedancePassword) await must(window.h3.setSecret("seedancePassword", seedancePassword)); if (sshPassword) await must(window.h3.setSecret("sshPassword", sshPassword)); setSettings(saved); setApiKey(""); setSeedanceUsername(""); setSeedancePassword(""); setSshPassword(""); onNotice("连接设置已安全保存"); } catch (e) { onError(e); } }
  ...
  return <section className="page connections"><div className="page-title"><div><span className="eyebrow">CONNECT</span><h1>连接设置</h1></div><button className="primary" onClick={save}>保存全部设置</button></div>
    ...
    <ConnectionCard title="Seedance 2.0 API" badge="SEEDANCE" result={results.seedance} onTest={() => test("seedance")} testing={testing === "seedance"}><label>API 地址<input value={draft.seedanceBaseUrl} onChange={(e) => setDraft({...draft,seedanceBaseUrl:e.target.value})}/></label><div className="form-grid"><label>账号<input value={seedanceUsername} onChange={(e) => setSeedanceUsername(e.target.value)} placeholder="已保存的账号不会回显；留空不修改"/></label><label>密码<input type="password" value={seedancePassword} onChange={(e) => setSeedancePassword(e.target.value)} placeholder="使用系统安全存储"/></label></div></ConnectionCard>
...
function backendLabel(kind: BackendKind) { return kind === "local" ? "本机 ComfyUI" : kind === "ssh" ? "SSH 远程显卡" : kind === "seedance" ? "Seedance 2.0 API" : "MiniMax H3 云 API"; }
