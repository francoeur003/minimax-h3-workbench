...
一个面向 MiniMax H3 的桌面工作台，把配置检测、模型下载、ComfyUI 配置、四路视频生成、本地/云端/SSH 连接放到同一个软件里。
...
- 内嵌官方模型与工作流下载地址，支持断点续传、下载进度、取消和目标目录安装。
...
模型权重不包含在软件安装包中。首次使用请进入“模型下载”，阅读并接受模型仓库中的 Community License 后再下载。
...
- 本软件不会附带或代发模型权重，模型许可证与费用由使用者自行确认。
